...
// No restriction on vertical size for new tab panel
if ( /\?fs\&/.test(window.location.search) ) {
...
var renderPrivacyExposure = function() {
    ...
    uDom.nodeFromId('popupHitDomainCount').textContent = touchedDomainCount.toLocaleString() + ' of ' + allDomainCount.toLocaleString();
};
...
/* must comment out because 'gotoZap' removed from popup.html
...
var renderOnce = function() {
    ...
    uDom.nodeFromId('appname').textContent = 'xBA';
...
*/
    if ( document.body.classList.contains('fullsize') === false ) {
        var lpane = uDom.nodeFromSelector('#panes > div:last-of-type'),
            height = (typeof popupData.height === 'string' && popupData.height !== 'unset') ? popupData.height : '500px';
        lpane.style.setProperty('height', height);
    }
};
    ...
    if ( ev.shiftKey && ev.ctrlKey ) {
        messaging.send(
            'popupPanel',
            {
                what: 'gotoURL',
                details: {
                    url: 'popup.html?fs&tabId=' + popupData.tabId,
                    select: true,
                    index: -1
                }
            }
