...
var nodeFromURL = function(url, re) {
    ...
    //console.log('%s at %s', re.source, url);
    if ( /^\\?[/.-_?&=][a-z0-9%]/.test(re.source) === true ) {
        while ( matches !== null ) {
            // need case-sensitive tests here, unlike #3011
            if ( /[a-zA-Z0-9%]/.test(url.charAt(re.lastIndex)) === false || 
                 /[a-zA-Z0-9%]/.test(url.charAt(re.lastIndex - 1)) === false ) {
                break;
            }
            matches = re.exec(url);
        }
        if ( matches === null ) {
            console.log('\tlogger FAILED to find word boundary for %s at %s', re.source, url);
            return document.createTextNode(url);
        }
    }
...
var reverseLookupManager = (function() {
    ...
    var reverseLookupDone = function(response) {
        ...
        var found = false,
            dialog = filterFinderDialog.querySelector('.dialog');
        ...
        if ( !found ) { // TODO: convert to new i18n message
            dialog.appendChild(document.createTextNode('Filter not found in any list.'));
        }
