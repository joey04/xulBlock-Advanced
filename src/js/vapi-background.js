...
var tabWatcher = (function() {
    ...
    var attachToTabBrowser = function(window) {
        ...
        if ( tabBrowser.deck ) {                    // Fennec
            ...
            if ( window && window.document.readyState === 'complete' ) {
                // register xBA hotkeys
                //   need event listeners since setAttribute fires oncommand immediately
                var doc = window.document;
                if ( doc.getElementById('keyset_xBA') === null ) {
                    var uKeyset = doc.createElement('keyset');
                    uKeyset.setAttribute('id', 'keyset_xBA');
                    doc.getElementById('mainKeyset').parentNode.appendChild(uKeyset);

                    // Alt+P for the Panel
                    var uKeyPan = doc.createElement('key');
                    uKeyPan.setAttribute('id', 'key_xBA_panel');
                    uKeyPan.setAttribute('key', 'P');
                    uKeyPan.setAttribute('modifiers', 'alt');
                    uKeyPan.setAttribute('oncommand', 'void(0);');
                    uKeyPan.addEventListener('command', vAPI.toolbarButton.panelHotkey);
                    uKeyset.appendChild(uKeyPan);

                    // Ctrl+Shift+L to open Logger for current tab
                    var uKeyLog = doc.createElement('key');
                    uKeyLog.setAttribute('id', 'key_xBA_logger');
                    uKeyLog.setAttribute('key', 'L');
                    uKeyLog.setAttribute('modifiers', 'control shift');
                    uKeyLog.setAttribute('oncommand', 'void(0);');
                    uKeyLog.addEventListener('command', vAPI.toolbarButton.loggerHotkey);
                    uKeyset.appendChild(uKeyLog);
                }
            }
        }
    ...
    var onWindowUnload = function(win) {
        ...
        var xBAkeyset = win.document.getElementById('keyset_xBA');
        if ( xBAkeyset !== null ) {
            xBAkeyset.parentNode.removeChild(xBAkeyset);
        }
...
var httpObserver = {
    ...
    handleRequest: function(channel, URI, details) {
        ...
        // Request will be sent, so filter its Referer header
        var referer = this.getRequestHeader(channel, 'referer'),
            logToConsole = µBlock.logger.isEnabled();

        if ( referer !== undefined ) {
            // https://github.com/joey04/xulBlock-Advanced/wiki/HTTP-Referer-spoofing
            //   can only modify Referer for 3rd-party requests
            //   spoofed with recipient hostname to conceal the actual referer

            var requestURI = (logToConsole && URI.asciiSpec.length > 100 && µBlock.hiddenSettings.logTruncatedRefererResource) ? URI.asciiSpec.substring(0, 100) + '…' : URI.asciiSpec,
                requestHostname = µBlock.URI.hostnameFromURI(requestURI),
                toDomain = µBlock.URI.domainFromHostname(requestHostname);

            if ( toDomain !== '' && toDomain !== µBlock.URI.domainFromURI(referer) )
            {
                if ( µBlock.hnSwitches.evaluateZ('spoof-3p-referer-to', requestHostname) )
                {
                    var noerr = this.setRequestHeader( channel, 'referer', (µBlock.URI.schemeFromURI(requestURI) + '://' + requestHostname + '/') );
                    if ( logToConsole ) {
                        if ( noerr )
                            console.log('Spoofed referer  %s\n\tto  %s', referer, requestURI);
                        else
                            console.log('ERROR setting Referer header');
                    }
                } else if ( logToConsole && µBlock.hiddenSettings.logEveryHttpReferer ) {
                    console.log('Unmodified referer  %s\n\tspoof-3p-referer-to: false or unset for  %s', referer, requestURI);
                }
            } else if ( logToConsole && µBlock.hiddenSettings.logEveryHttpReferer ) {
                console.log('referer  %s\n\tUnmodified to  %s', referer, requestURI);
            }
        } else if ( logToConsole && µBlock.hiddenSettings.logEveryHttpReferer ) {
            console.log('no referer to  %s', ((URI.asciiSpec.length > 100 && µBlock.hiddenSettings.logTruncatedRefererResource) ? URI.asciiSpec.substring(0, 100) + '…' : URI.asciiSpec));
        }
    ...
    setRequestHeader: function(channel, name, value) {
        try {
...
(function() {
    ...
    tbb.loggerHotkey = function() {
        var win = winWatcher.getCurrentWindow();
        var curTabId = tabWatcher.tabIdFromTarget(getTabBrowser(win).selectedTab);
        vAPI.tabs.open({url: 'logger-ui.html#tab_' + curTabId, index: -1, select: true});
    };

    tbb.panelHotkey = function() {
        if ( µBlock.hiddenSettings.popupIsHotkey === true ) {
            tbb.panelPopupHotkey();
        } else {
            tbb.panelTabHotkey();
        }
    };

    tbb.panelTabHotkey = function() {
        var win = winWatcher.getCurrentWindow();
        var curTabId = tabWatcher.tabIdFromTarget(getTabBrowser(win).selectedTab);
        vAPI.tabs.open({url: 'popup.html?fs&tabId=' + curTabId, index: -1, select: true});
    };

    tbb.panelPopupHotkey = function() {
        var win = winWatcher.getCurrentWindow();
        var tbbPanel = win.document.getElementById(tbb.viewId);
        // note: onViewShowing/Hiding will execute and thus exclusively control isPopupOpen
        if ( tbb.isPopupOpen === false ) {
            tbbPanel.showPopup();
        } else {
            tbbPanel.hidePopup();
        }
